...
class ProfileModel {
        ...
        try {
            ...
            if (!user_id) {
                return res.status(401).json({
                    message: `You do not registered yet`
                });
            }

            const [profileFind] = await conn.query("Select * from profiles where user_id = ?", [user_id]);
    ...
    static async putBanner(req = request, res = response) {
        ...
        try {
            ...
            const urlBanner = await uploadPicture(banner.tempFilePath)
                .then(picture => {
                    return picture.secure_url
                });
